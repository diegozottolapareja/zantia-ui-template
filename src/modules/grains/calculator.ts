import type { Posicion } from './data/posiciones'
import type { Necesidad } from './data/necesidades'
import type { Producto } from './data/tipos'
import type { Parametros } from './ParametrosContext'
...
export function calcular(input: CalcInput, params: Parametros): CalcBreakdown {
  ...
  const ajusteGrado = (grado - 1) * 0.005 * fasComprador
  ...
  const lineas: LineaBreakdown[] = [
    { concepto: 'FAS comprador', valorTn: fasComprador, detalle: 'Precio ofrecido en destino' },
    { concepto: `Flete (${distanciaKm} km → puerto)`, valorTn: -flete, detalle: `Base USD ${params.fleteBase} + USD ${params.fletePorKm}/km` },
  ]

  if (descuentoHumedad > 0) {
    lineas.push({ concepto: `Secado / humedad (${excessHumedad.toFixed(1)}% exceso)`, valorTn: -descuentoHumedad, detalle: `Humedad ${humedad}% vs estándar ${humStd}%` })
  }
  if (descuentoZarandeo > 0) {
    lineas.push({ concepto: 'Zarandeo / mermas', valorTn: -descuentoZarandeo })
  }
  if (ajusteGrado > 0) {
    lineas.push({ concepto: `Ajuste grado (G${grado})`, valorTn: -ajusteGrado, detalle: 'Descuento 0.5% por grado por debajo de G1' })
  }
  lineas.push({ concepto: `Comisión corredor (${(params.comisionPorc * 100).toFixed(1)}%)`, valorTn: -comisionTn })
  ...
  return {
    fasComprador, volumen, distanciaKm, lineas,
    flete, descuentoHumedad, descuentoZarandeo, ajusteGrado,
    comisionTn, netoPorTn, netoTotal, comisionTotal,
    cumpleCalidad: true,
    fobReferencia: fobRef, retencionesTn, fasTeoricoTn, deltaFas,
    aceptaPrecio, diferenciaPrecio,
  }
}
